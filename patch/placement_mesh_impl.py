...
class PlacementMeshImpl(mtf.MeshImpl):
  ...
  def __init__(self, shape, layout, devices, conv_shape, fil, speeds):
    super(PlacementMeshImpl, self).__init__(shape, layout, conv_shape, fil, speeds)
  ...
  class LaidOutTensor(object):
    ...
    @property
    def slice_shape(self, xnum=0):
      y = self.tensor_list[0].shape.as_list()
      return y
  ...
  class LaidOutVariable(object):
    ...
    def __init__(self, variable, mesh_impl):
      ...
      conv_shape = mesh_impl.get_conv_shape
      fil = mesh_impl.get_fil
      speeds = mesh_impl.get_speeds
      if self.slice_is_master:
        ...
        for pnum in xrange(mesh_impl.size):
          with tf.device(mesh_impl.devices[pnum]):
            slice_shape = mesh_impl.slice_shape(shape, this=1)
        ...
        if os.environ.get("MTF_SEQUENCE_MODE", "") == "1":
          ...
          self._copy_master_to_slices = self.assign_to_slices(
              mtf.assign_slice,
              mesh_impl.make_slices(variable.get_master(), shape, conv_shape, fil, speeds))
        self._copy_slices_to_master = variable.assign_to_master(
            mesh_impl.combine_slices(slices_with_master_dtype, shape, fil=fil))
  ...
  def export_to_tf_tensor(self, x, laid_out_x):
    ...
    Returns:
      a tf.Tensor
    """
    return self.combine_slices(laid_out_x.all_slices, x.shape, fil=self.get_fil)
  ...
  def import_tf_tensor(self, x, tf_x):
    ...
    Returns:
      a LaidOutTensor
    """
    return self.LaidOutTensor(self.make_slices(tf_x, x.shape, conv_shape=[30,20,10]))
