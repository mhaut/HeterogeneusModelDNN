...
tf.flags.DEFINE_integer("train_epochs", 5, "Total number of training epochs.")
tf.flags.DEFINE_integer("epochs_between_evals", 1,
                        "# of epochs between evaluations.")
tf.flags.DEFINE_integer("eval_steps", 0,
                        "Total number of evaluation steps. If `0`, evaluation "
                        "after training is skipped.")
tf.flags.DEFINE_string("mesh_shape", "b1:3;b2:1;b3:1", "mesh shape")
tf.flags.DEFINE_string("layout", "filters1:b1;filters2:b2;filters3:b3",
                       "layout rules")
...
def mnist_model(image, labels, mesh):
  ...
  f1 = mtf.relu(mtf.layers.conv2d_with_blocks(
      x, filters1_dim, filter_size=[7, 7], strides=[1, 1], padding="SAME",
 ...
 # f1 = mtf.reshape(f1, [FLAGS.batch_size, 1, 30, 3, 10, 1])
  ...
  f2 = mtf.relu(mtf.layers.conv2d_with_blocks(
      f1, filters2_dim, filter_size=[5, 5], strides=[1, 1], padding="SAME",
  ...
  f3 = mtf.relu(mtf.layers.conv2d_with_blocks(
      f2, filters3_dim, filter_size=[3, 3], strides=[1, 1], padding="SAME",
...
def heterogeneousPartition(blocks):
  ...
  for num_fil in blocks:
    ...
    for j in num_fil:
      ...
      w_block+=1
  all_filters_blocks = []
  for i in range(0,len(all_filters)-1, 3):
    all_filters_blocks.append([all_filters[i], all_filters[i+1], all_filters[i+2]])
...
def model_fn(features, labels, mode, params):
  ...
  logits, loss, all_filters = mnist_model(features, labels, mesh)
