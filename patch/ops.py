    ...
    for op in operations:
      if isinstance(op, StackedVariable):
        ...
        if mesh_to_impl is not None:
          ...
          slice_size = mesh_impl.slice_size(op.shape, test=1)
          num_to_stack = min(
...
class Lowering(object):
  ...
  def verify_slice_shapes(self, tensor, laid_out_tensor):
    ...
    #correct_shape = mesh_impl.slice_shape(tensor.shape, this=1)
...
class MeshImpl(object):
  ...
  def __init__(self, shape, layout_rules, conv_shape, fil, speeds):
    ...
    self._fil = fil
    self._pos = 0
    self._temporal_block = 0
    self._block = 0
    self._speeds = speeds
    self._receive_block = 0
    self._not_block_check = 0
    self._replicated = 0
    self.block_to_combine = 0
    self._acum = 0
  ...
  def get_combine(self):
    return self.block_to_combine

  @property
  def get_not_block_check(self):
    return self._not_block_check

  @property
  def get_replicated(self):
    return self._replicated

  @property
  def get_receive_block(self):
    return self._receive_block
...
#    self._block+=1
  ...
  @property
  def set_temporal_block(self, block):
    self._temporal_block=block

  @property
  def get_temporal_block(self):
    return self._temporal_block
  ...
  def slice_shape(self, tensor_shape, item=None, test=None, pnum=None, this=None, conv_shape=None, full=None):
    ...
    for tensor_dim, mesh_axis in zip(
      ...
      if mesh_axis is None:
        ...
        if(pnum!=None):
          ret.append(conv_shape[block][(int(tensor_dim.name[7])-1)][pnum])
        elif(full==True):
          ret.append(tensor_dim.size)
          self._pos=0
        elif(this==1):
          if (int(tensor_dim.name[7])-1)%2 != 0:
            ret.append(self.get_conv_shape[block][(int(tensor_dim.name[7])-1)][self._pos-1])

          else:
            ret.append(self.get_conv_shape[block][(int(tensor_dim.name[7])-1)][self._pos])
            if(this==1 or this==2):
              self._pos+=1
              if(self._pos==len(self.get_conv_shape[block][(int(tensor_dim.name[7])-1)])):
                self._pos=0
        elif(item!=None):
          ret.append(item)
        else:
  ...
  def slicing(self, tensor_shape, pnum, conv_shape=None, block=None):
    ...
    for tensor_dim, mesh_axis in zip(
      ...
      if mesh_axis is None:
        ...
        ret.append(conv_shape[block][(int(tensor_dim.name[7])-1)][pnum])
        dim = int(tensor_dim.name[7])-1
        self.block_to_combine = block
    return ret, dim

  def get_speeds(self, tensor, speeds, pos):
    l = []
    for i in range(0, len(speeds)):
      value = int(tensor*speeds[i])
      if value < 1:
        l.append(1)
      else:
        l.append(value)
  ...
  def slice_begin(self, tensor_shape, pnum, dim=None, conv_shape=None, fil=None, block=None, speeds=None, shape=None):
...
#    flag_filters=3 if str(self.tensor_layout(tensor_shape)) != "TensorLayout(None, None, None, 0)" else -1
    ...
    if index_0 != None and int(shape[-2])-tensor!=0:
      if pnum==0 or tensor_shape[-1].name == 'hidden1':
        conv.append(0)
        if int(shape[-2])-tensor!=0:
          self._acum+=int(shape[-2])
      else:
        conv.append(self.get_acum)
        self._acum+=int(shape[-2])
    else:
      conv.append(0)
    ...
    if flag_filters ==3:
      if pnum==0 or tensor_shape[-1].name == 'hidden1':
        conv2.append(0)
        self._acum+=int(shape[-1])
      else:
        conv2.append(self.get_acum)
        self._acum+=int(shape[-1])
    else:
      conv2.append(0)
  ...
  def slice_size(self, tensor_shape, test=None):
    y = list_product(self.slice_shape(tensor_shape))
    #y = list_product(self.slice_shape(tensor_shape, conv_shape=[30,20,10], this=3))
    return y

  def laid_out_size(self, tensor_shape, this=None):
    """Total size of all slices.
    ...
    for i in range(0,3,1):
      l = self.slice_shape(tensor_shape, this=2)
      for j in l:
        lista.append(j)
    return list_product(lista)
  ...
  def allsplit(self, x, mesh_axis, split_axis, which=None):
    ...
    for i in self._speeds:
      _slices.append(int(x.slice_shape[-1]*i))
    _slices[np.argmin(_slices)]=1 if(_slices[np.argmin(_slices)] < 1) else _slices[np.argmin(_slices)]
    while sum(_slices) < x.slice_shape[-1]:
      _slices[np.argmax(_slices)]+=1

    while sum(_slices) > x.slice_shape[-1]:
      _slices[np.argmax(_slices)]+=1

    def my_fn(x, which, _slices):
      slice_begin = [
      ...
      slice_size = [
          ...
          for i, dimsize in enumerate(x.shape.as_list())]
      slice_size[-1] = _slices[which]
      return tf.slice(x, slice_begin, slice_size)
    return self.slicewise(my_fn, x, which, _slices)
  ...
  def broadcast_impl(self, old_slices, old_shape, new_shape):
    ...
    Returns:
      LaidOutTensor.
    """
    new_slice_shape = self.slice_shape(new_shape, this=5)
    def tf_fn(x):
      return (tf.zeros(new_slice_shape, dtype=x.dtype) +
              _expand_dims(x, old_shape, new_shape))
    return self.slicewise(tf_fn, old_slices)

  def get_block_filters(self, tf_tensor):
    block = self.get_block
    return block

  def make_slices(self, tf_tensor, tensor_shape, conv_shape=None, fil=None, speeds=None):
    ...
    def my_fn(pnum):
      if tensor_layout.is_fully_replicated:
        block = self.get_block_filters(tf_tensor)
        slice_shape, _ = self.slicing(tensor_shape, pnum, conv_shape, block)
        slice_shape = self.slice_shape(tensor_shape, full=True)
        self._temporal_block = self.get_block
        return tf_tensor
      else:
        block = self.get_block_filters(tf_tensor)
        slice_shape, dim = self.slicing(tensor_shape, pnum, conv_shape, block)
        slice_begin = self.slice_begin(tensor_shape, pnum, dim, fil=fil, block=self._block, speeds=speeds, shape=slice_shape)
        ar_zeros = np.zeros((len(slice_begin),), dtype=int)
        if np.array_equal(ar_zeros,np.array(slice_begin)):
          self._replicated=1
        else:
          self._replicated=0
        return tf.slice(tf_tensor, slice_begin, slice_shape)

    y = parallel([tf_tensor.device] * self.size, my_fn,
                    list(xrange(self.size)))
    return y



  def combine_slices(self, slices, tensor_shape, device=None, fil=None):
    if tensor_shape.ndims == 0:
      return slices[0]
    ret = slices[:]
    tensor_layout = self.tensor_layout(tensor_shape)

    block = self.get_combine
    conv_f = fil[block]
    ...
    for mesh_dim, tensor_axis in zip(
      ...
      if tensor_axis is None:
        ...
        if self.get_replicated == 1:
          self._replicated=0
          return slices[0]
        elif conv_f[int(tensor_layout[-1])]==concat_inputs[0][0].shape[-1] and tensor_axis < 4 and slices[0].shape != [3, 3, 1, 60]:
          tensor_axis=2
        else:
          tensor_axis=3
        if len(concat_inputs[0][0].shape)==5:
          tensor_axis+=1
        if len(concat_inputs[0][0].shape)<4:
          tensor_axis=2
        ret = parallel(
            devices, tf.concat, [slices],
  ...
  def lower(self, lowering):
    ...
    def tf_fn(tf_input, tf_filter):
      output = tf.nn.conv2d(
          ...
          tf_filter, [1, 1], self._padding)
      return _tf_restore_batch_dims(output, 3, tf_input, 1)
    y = mesh_impl.slicewise(
...
class Conv2or3dBackpropInputOperation(Operation):
  ...
  def lower(self, lowering):
    mesh_impl = lowering.mesh_impl(self)
    self._conv = mesh_impl.get_conv_shape
    dy, conv_filter = self.inputs
    input_sizes = mesh_impl.slice_shape(self.outputs[0].shape, conv_shape=self._conv, this=5)
    ...
    if self._is_transpose:
      ...
      def tf_fn(tf_dy, tf_filter):
        return _tf_restore_batch_dims(
            ...
            self._num_nonbatch_dims, tf_dy, 2)
      dx = mesh_impl.slicewise(
      ...
      def tf_fn(tf_dy, tf_filter):
        input_sizes = mesh_impl.slice_shape(self.outputs[0].shape, conv_shape=self._conv, this=1)
        input_sizes = [list_product(input_sizes[:-self._num_nonbatch_dims])] + (
            input_sizes[-self._num_nonbatch_dims:])
        y = _tf_restore_batch_dims(
            backprop_fn(
                input_sizes, tf_filter,
                _tf_flatten_batch_dims(tf_dy, self._num_nonbatch_dims),
                self._strides, self._padding),
            self._num_nonbatch_dims, tf_dy, 3, self._conv, self._pos)
        self._pos+=1
        if self._pos==3:
          self._pos=0
        return y
...
class Conv2or3dBackpropFilterOperation(Operation):
  ...
  def lower(self, lowering):
    ...
    def tf_fn(tf_input, tf_dy):
      filter_sizes = mesh_impl.slice_shape(self.outputs[0].shape, this=1)
      if self._is_transpose:
        y, x = tf_input, tf_dy
      else:
        x, y = tf_input, tf_dy
...
class Conv3dOperation(Operation):
  ...
  def lower(self, lowering):
    ...
    def tf_fn(tf_input, tf_filter):
      ...
      return _tf_restore_batch_dims(output, 4, tf_input, 4)
    y = mesh_impl.slicewise(
        tf_fn, lowering.tensors[conv_input], lowering.tensors[conv_filter])
    # reducing out input channels - may need to allreduce
    in_mesh_axis = mesh_impl.tensor_dimension_to_mesh_axis(self._in_dim)
  ...
  def lower(self, lowering):
    ...
    def tf_fn(tf_input, tf_filter):
      ...
      output = tf.nn.conv2d_backprop_input(
          ...
          _tf_flatten_batch_dims(tf_input, 3),
          self._strides, self._padding)
      return _tf_restore_batch_dims(output, 3, tf_input, 5)
  ...
  def lower(self, lowering):
    ...
    def tf_fn(tf_input, tf_filter):
      ...
      output = conv3d_backprop_input_v2(
          ...
          _tf_flatten_batch_dims(tf_input, 4),
          self._strides, self._padding)
      return _tf_restore_batch_dims(output, 4, tf_input, 6)
...
class OneHotOperation(Operation):
  ...
  def lower(self, lowering):
    ...
    output_shape = self.outputs[0].shape
    output_slice_shape = mesh_impl.slice_shape(output_shape, this=5)
  ...
  def lower(self, lowering):
    ...
    lowering.set_tensor_lowering(
        ...
        LazyLaidOutTensor(to_laid_out_tensor_fn,
                          mesh_impl.slice_shape(self.shape, conv_shape=mesh_impl.get_conv_shape,this=5)))
    if self._trainable:
...
class Constant(Operation):
  ...
  def lower(self, lowering):
    mesh_impl = lowering.mesh_impl(self)
    slice_shape = mesh_impl.slice_shape(self.outputs[0].shape, this=5)
    def tf_fn():
...
def _tf_restore_batch_dims(x, num_nonbatch_dims, prototype, this=None, conv=None, pos=None):
